        ...
        <ul>
          <li v-for="item in articles" :key="item.id">
            <div v-if="item.is_release" class="rounded overflow-hidden shadow-lg bg-white my-4 cursor-pointer" @click="goToDetailPost(item.id)">
              <img class="w-full h-64 object-cover transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-105" :src="`https://jamstack-api.herokuapp.com${item.image.url}`" :alt="item.title">
              <div class="px-4 py-4">
